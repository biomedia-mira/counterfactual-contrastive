"""
Script to run counterfactual finetuning for CXR model
"""
...
class CounterfactualTrainingModule(pl.LightningModule):
    ...
    def common_step(self, batch: BatchType) -> Any:  # type: ignore
        with torch.no_grad():
            ...
            cf_pa["scanner"] = torch.randint_like(cf_pa["scanner"], high=2)
            cf_pa["sex"] = torch.randint_like(cf_pa["sex"], high=2)
        ...
        sex_intervened = self.model_dict["sex"](cf_x)
        sex_real = cf_pa["sex"]
        aux_loss = self.criterion(
            scanner_intervened, scanner_real.squeeze(1)
        ) + self.criterion(sex_intervened, sex_real.squeeze(1))
...
if __name__ == "__main__":
    ...
    model_paths = {
        "scanner": "/vol/biomedic3/mb121/causal-contrastive/outputs2/run_d2l8vxwm/best.ckpt",
        "sex": "/vol/biomedic3/mb121/causal-contrastive/outputs2/run_9xnpe3w2/best.ckpt",
    }
